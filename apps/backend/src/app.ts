...
export function buildApp(prismaClient?: PrismaClient) {
  ...
  fastify.post("/api/partidas", async (request, reply) => {
    ...
    if (!jugador) {
      return reply
        .status(404)
        .send({ error: "Jugador no encontrado. Créalo primero." });
    }
  ...
  fastify.get("/api/ranking", async (request, reply) => {
    const requestedLimit = parseIntegerInRange(
      (request.query as { limit?: unknown })?.limit ?? 10,
      1,
      MAX_LIMIT,
    );
  ...
  fastify.get("/api/stats", async (request, reply) => {
    ...
    return {
      ...
      mediaTiempo: agregados._avg.tiempo
        ? Math.round(agregados._avg.tiempo)
        : 0,
      jugadorMasRapido: recordAbsoluto
        ? recordAbsoluto.jugador.nombre
        : "Nadie aún",
